...
app.get('/', sessionMiddleware, zValidator('query', workspaceQuerySchema), async (ctx) => {
...
app.get('/:employeeId', sessionMiddleware, async (ctx) => {
  const user = ctx.get('user');
...
app.post('/', sessionMiddleware, zValidator('json', createEmployeeSchema), async (ctx) => {
  const user = ctx.get('user');
...
app.patch('/:employeeId', sessionMiddleware, zValidator('json', updateEmployeeSchema), async (ctx) => {
  const user = ctx.get('user');
  const { employeeId } = ctx.req.param();
...
app.delete('/:employeeId', sessionMiddleware, async (ctx) => {
  const user = ctx.get('user');
  const { employeeId } = ctx.req.param();
