...
app.get('/', sessionMiddleware, async (ctx) => {
...
app.post('/', zValidator('form', createWorkspaceSchema), sessionMiddleware, async (ctx) => {
  const user = ctx.get('user');
...
app.get('/:workspaceId/info', sessionMiddleware, async (ctx) => {
  const { workspaceId } = ctx.req.param();
...
app.patch('/:workspaceId', sessionMiddleware, zValidator('form', updateWorkspaceSchema), async (ctx) => {
  const user = ctx.get('user');
  const { workspaceId } = ctx.req.param();
  const { name, image } = ctx.req.valid('form');
...
app.delete('/:workspaceId', sessionMiddleware, async (ctx) => {
  const user = ctx.get('user');
  const { workspaceId } = ctx.req.param();
...
app.post('/:workspaceId/resetInviteCode', sessionMiddleware, async (ctx) => {
  const user = ctx.get('user');
  const { workspaceId } = ctx.req.param();
...
app.get('/:workspaceId/analytics', sessionMiddleware, async (ctx) => {
  const user = ctx.get('user');
  const { workspaceId } = ctx.req.param();
